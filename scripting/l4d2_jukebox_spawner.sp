...
public void OnPluginStart()
{
	...
	g_hCvarAllow = CreateConVar(	"l4d2_jukebox_allow",			"1",			"0=Plugin off, 1=Plugin on.", CVAR_FLAGS );
	g_hHordeMode = CreateConVar(	"l4d2_jukebox_horde_modes",		"",				"Create a panic event on these gamemodes when track #4 plays. Separate by commas (no spaces). \"\" = all.", CVAR_FLAGS );
	g_hHordeNotify = CreateConVar(	"l4d2_jukebox_horde_notify",	"1",			"Display a chat message when the horde is triggered by track #4.", CVAR_FLAGS );
	g_hHordeTime = CreateConVar(	"l4d2_jukebox_horde_time",		"42",			"How many seconds after track #4 plays before triggering a horde. Valve default 42.", CVAR_FLAGS, true, 0.0, true, 60.0 );
	g_hHordeTrig = CreateConVar(	"l4d2_jukebox_horde_trigger",	"-1",			"-1=Infinite, 0=Off. Specify how many times a panic event may occur when track #4 plays.", CVAR_FLAGS );
	g_hCvarModes = CreateConVar(	"l4d2_jukebox_modes",			"",				"Turn on the plugin in these game modes, separate by commas (no spaces). (Empty = all).", CVAR_FLAGS );
	g_hCvarModesOff = CreateConVar(	"l4d2_jukebox_modes_disallow",	"",				"Turn off the plugin in these game modes, separate by commas (no spaces). (Empty = none).", CVAR_FLAGS );
	g_hCvarModesTog = CreateConVar(	"l4d2_jukebox_modes_tog",		"0",			"Turn on the plugin in these game modes. 0=All, 1=Coop, 2=Survival, 4=Versus, 8=Scavenge. Add numbers together.", CVAR_FLAGS );
...
Action CmdJukeTracks(int client, int args)
{
	...
	if( client == 0 )
		...
		PrintToChat(client, "---------- Randomly Selected Tracks ----------");
	...
	if( client == 0 )
		...
		PrintToChat(client, "---------- All Random Tracks ----------");
	...
	if( client == 0 )
		...
		PrintToChat(client, "---------- All Main Tracks ----------");
	...
	if( client == 0 )
		...
		PrintToChat(client, "---------- Map Override Tracks ----------");
...
Action CmdJuke(int client, int args)
{
	...
	{
		PrintToChat(client, "%sError: Cannot add anymore jukeboxes. Used: (%d/%d).", CHAT_TAG, MAX_JUKEBOXES, MAX_JUKEBOXES);
		return Plugin_Handled;
	}
	...
	{
		PrintToChat(client, "%sCannot place jukebox, please try again.", CHAT_TAG);
		return Plugin_Handled;
	}
	else if( iJukeboxCount >= MAX_JUKEBOXES )
	{
...
Action CmdJukeBox(int client, int args)
{
	...
	if( hFile == null )
	{
		PrintToChat(client, "%sError: Cannot load the jukebox config (%s).", CHAT_TAG, CONFIG_SPAWNS);
		return Plugin_Handled;
	}
	...
	{
		PrintToChat(client, "%sError: Failed to add map to jukebox spawn config.", CHAT_TAG);
		delete hFile;
		return Plugin_Handled;
	}
	...
	{
		PrintToChat(client, "%sError: Cannot add anymore jukeboxes. Used: (%d/%d).", CHAT_TAG, iCount, MAX_JUKEBOXES);
		delete hFile;
		return Plugin_Handled;
	}
...
Action CmdJukeDelete(int client, int args)
{
	...
	PrintToChat(client, "%s(%d/%d) - Jukebox removed from config, add new jukeboxes with sm_jukebox.", CHAT_TAG, iCount, MAX_JUKEBOXES);
	return Plugin_Handled;
}
...
Action CmdJukeWipe(int client, int args)
{
	...
	{
		PrintToChat(client, "%sCannot find map in jukebox config.", CHAT_TAG);
		delete hFile;
		return Plugin_Handled;
	}
	...
	PrintToChat(client, "%s(0/%d) - All jukeboxes removed from config, add new jukeboxes with sm_jukebox.", CHAT_TAG, MAX_JUKEBOXES);
	return Plugin_Handled;
}
...
Action CmdJukeNext(int client, int args)
{
	...
	if( iJuke )
	{
		PrintToChatAll("%s\x05%N \x01changed tracks.", CHAT_TAG, client);
		AcceptEntityInput(iJuke, "Press");
	}
	else
	{
		PrintToChat(client, "%You are not near to a jukebox.", CHAT_TAG);
	}
	return Plugin_Handled;
}
...
Action CmdJukeStop(int client, int args)
{
	...
	PrintToChatAll("%s\x05%N \x01stopped jukebox playing.", CHAT_TAG, client);
	StopAllSound();
	return Plugin_Handled;
}
